...
The end screen rearranges on its side: the result across the top at full width, the buttons
sharing one row beneath it. It is the tallest panel in the game and stacked in one column it was
483px in a 390px viewport — it scrolled, with Main menu below the fold. Two side-by-side columns
fixed the height and cost the thing the screen is *for*, putting the score in a narrow column no
more prominent than the buttons beside it; landscape has width to spare and not height, so the
width goes to the result. Its two halves are `display: contents` in portrait, so that layout is
untouched — the panel sees the same eleven children with the same gaps, and dropping the wrappers
in without that quietly tightened it by 84px.
